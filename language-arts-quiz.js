...
const STORAGE_KEY = 'la-midyear-quiz-progress-v1';
const bank = window.LA_MIDYEAR_QUESTION_BANK || { questions: [], metadata: {} };
...
function loadState() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEY)) || { answers: {}, order: [], streak: 0 };
  } catch {
    return { answers: {}, order: [], streak: 0 };
  }
}
...
function render() {
  ...
  if (!visible.length) {
    els.number.textContent = 'No questions';
    els.topic.textContent = 'Adjust filters';
    els.difficulty.textContent = 'Ready';
    els.prompt.textContent = 'No questions match the current filter.';
  ...
  els.number.textContent = `Question ${activeIndex + 1} of ${visible.length}`;
...
function renderShortAnswer(question, record) {
  els.answerArea.innerHTML = `
    <textarea name="answer" maxlength="700" placeholder="Write a concise exam-style answer.">${escapeHtml(record?.answer || '')}</textarea>
  `;
}
...
function submitAnswer(event) {
  ...
  if (!answer) {
    ...
    els.feedback.innerHTML = '<h3>Answer needed</h3><p>Choose an option or write a response before submitting.</p>';
    return;
  }
...
function grade(question, answer) {
  ...
  const hasAnalysisTerm = /(symbol|imagery|foil|juxtaposition|irony|allegory|propaganda|character|conflict|catalyst|tone|theme)/i.test(answer);
  const enoughLength = answer.split(/\s+/).filter(Boolean).length >= 20;
  const raw = (hits.length / question.requiredKeywords.length) * 0.65 + (hasAnalysisTerm ? 0.2 : 0) + (enoughLength ? 0.15 : 0);
...
function renderFeedback(question, record) {
  ...
  const title = record.score >= 1 ? 'Correct' : record.score >= 0.5 ? 'Partly there' : 'Needs revision';
  const correction = question.type === 'multiple-choice'
    ? `<p><b>Correct answer:</b> ${escapeHtml(question.correctAnswer)}</p>`
    : `<p><b>Model answer:</b> ${escapeHtml(question.sampleAnswer)}</p>`;
  const shortTip = question.type === 'short-answer'
    ? `<p><b>Auto-check:</b> This response earned ${Math.round(record.score * 100)}%. Include the expected concept words, a precise literary term, and a clear why-it-matters explanation.</p>`
    : '';
  ...
  els.feedback.innerHTML = `
    ...
    ${correction}
    <p><b>Correction:</b> ${escapeHtml(question.explanation)}</p>
    ${shortTip}
  `;
}
...
function updateMetrics() {
  ...
  els.progressText.textContent = `${progress}% complete. Progress is saved automatically in this browser.`;
}
...
function updateDiagnosis() {
  ...
  if (!items.length) {
    els.diagnosisList.innerHTML = '<p class="progressText">Submit answers to unlock personalised priorities.</p>';
    return;
  }
...
function diagnosisTip(domain, missed) {
  if (domain.includes('Purple Hibiscus')) return `Revisit STEAL, foils, symbolism, and how setting changes Kambili/Jaja. Missed: ${missed}.`;
  if (domain.includes('Animal Farm')) return `Practise allegory, propaganda, Boxer, Benjamin, Clover, and disillusionment. Missed: ${missed}.`;
  if (domain.includes('Rhetorical')) return `Drill ethos/logos/pathos and explain audience effect, not just definitions. Missed: ${missed}.`;
  if (domain.includes('Grammar')) return `Review form first, then choose by meaning: tense, clauses, passive, clefts, inversion. Missed: ${missed}.`;
  return `Use precise terms, micro-quotes, and direct analytical claims. Missed: ${missed}.`;
}
...
els.reset.addEventListener('click', () => {
  if (!confirm('Reset all saved quiz progress?')) return;
  state = { answers: {}, order: [], streak: 0 };
  saveState();
  render();
});
